...
`
    const url = config.otp2Url + '/routers/default/index/graphql'
    fetch(url, {body: routeQuery, method: 'POST'})
      .then(res => res.json())
      .then(response => {
        // write json to cache
...
function processFeed (feedData) {
  // process patterns
  const patterns = []
  for (let rte of feedData.data.routes) {
    for (let ptn of rte.patterns) {
      ptn.route_short_name = rte.shortName

      // convert to Global Mercator meters
      ptn.geometry.googlecoords = polyline.decode(ptn.patternGeometry.points)
      ptn.coordsGM = ptn.geometry.googlecoords.map(c =>
        GlobalMercator.latLonToMeters(c[0], c[1]))
  ...
  // process stops
  const stops = feedData.data.stops.map(stop => {
